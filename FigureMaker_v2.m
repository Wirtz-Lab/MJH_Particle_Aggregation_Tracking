...
prompt = {'Enter number of columns:','Enter number of rows:','time Increment','Replicates per condition:'}; 
titles = 'Plate Input';
definput = {'12','8','10','3'};
...
timeIncrement = str2double(Inputs{3,1});
Replicates = str2double(Inputs{4,1});

WellArray =[1:1:NumX*NumY];

plateOrder =zeros(NumY,NumX);

for  k = 1:NumX
    if k == 1 || rem(k,2)~=0;
        plateOrder(:,NumX-k+1) = WellArray([(NumY*k-NumY)+1:NumY*k]).';
    elseif k ~= 1 || rem(k,2) == 0;
        plateOrder(:,NumX-k+1) = flipud(WellArray([(NumY*k-NumY)+1:NumY*k]).'); 
    else k ~= 1 || rem(k,2) ~= 0;
        plateOrder(:,NumX-k+1) = WellArray([(NumY*k-NumY)+1:NumY*k]).';
    end
end
...
whitebg('w');
WellInputs = inputdlg('Based on heatmap, which wells are replicates?')
WellInputs2 = str2num(WellInputs{1});


% k=1;
% for k = 1:length(WellInputs2);
% var(strcat('welln',num2str(k))) = strcat('well',num2str(WellInputs2([k])))
% end    
Welln1 = strcat('well',num2str(WellInputs2([1])));
Welln2 = strcat('well',num2str(WellInputs2([2])));
Welln3 = strcat('well',num2str(WellInputs2([3])));

% y = strcat('welln',num2str(k))
% x = strcat('well',num2str(WellInputs2([3])));
